...
def create_discipline(d: DisciplineCreate):
    try:
        result = Discipline.add_discipline(name=d.name, code=d.code, is_active=d.is_active)
        return result
...
def update_discipline(disc_id: int, d: DisciplineUpdate):
    try:
        result = Discipline.update_discipline_by_id(
...
def delete_discipline(disc_id: int):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Внутренняя ошибка при удалении: {str(e)}")
